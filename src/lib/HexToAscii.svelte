<script lang="ts">
    ...
    const hexToAscii = (input: string) =>
        new TextDecoder().decode(hexToBytes(input));
    const asciiToHex = (input: string) =>
...
<div class="flex flex-col gap-3">
    ...
    <P>
        This coverter can be used to convert from a hex encoded string to its
        human redable representation and vice verca.
    </P>
    <div
        class="mt-3 flex flex-row items-center gap-3 text-2xl text-center font-bold"
    >
        <span class="text-gray-900 dark:text-white flex-1">
            {inputIsAscii ? "ASCII" : " HEX "}
        </span>
        <Button
            pill={true}
            outline={true}
            ...
            ><ArrowUpDownOutline size="lg" transform="rotate(90)" />
        </Button>
        <span class="text-gray-900 dark:text-white flex-1">
            {!inputIsAscii ? "ASCII" : " HEX "}
        </span>
    ...
    <div class="grid lg:grid-cols-2 grid-cols-1 gap-3">
        ...
        />
        {#if output.length === 0 || isASCII(output)}
            <ValueOutputWithCopy
                value={output}
                placeholder={inputIsAscii
                    ? "HEX will show up here"
                    : "ASCII will show up here"}
            ></ValueOutputWithCopy>
        {:else}
            <Alert color="red">
                This input does not decode to a printable ASCII string.
            </Alert>
        {/if}
    </div>
</div>
